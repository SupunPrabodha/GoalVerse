	...
	useEffect(() => {
		...
		async function load() {
			try {
				...
				const res = await fetch(`${API_BASE_URL}/volunteer/me`, {
				...
				if (res.ok) {
					...
					const data = await res.json().catch(() => ({}));
					setError(data.message || "Failed to load volunteer profile");
				}
			} catch (err) {
				if (!mounted) return;
	...
	return (
		<View style={styles.screen}>
			<ScrollView contentContainerStyle={styles.container}>
				...
				<Text style={styles.headerSub}>Manage your volunteer account preferences</Text>
				...
				{loading ? (
					...
					<View style={styles.card}>
						...
						<View style={styles.row}>
							<Image
								source={volunteerAvatar ? { uri: volunteerAvatar } : require("../../assets/images/logo.png")}
								style={styles.avatar}
							/>
						...
						<View style={styles.section}>
							...
							<Text style={styles.sectionText}>{profile?.total_hours ? `${profile.total_hours} hrs` : "—"}</Text>
						</View>
						{/* ...existing code... */}
						...
						<View style={[styles.settings]}> 
							...
							<TouchableOpacity style={styles.settingRow} onPress={() => router.push("/(tabs)/VolunteerHome") }>
								<View>
									<Text style={styles.settingTitle}>Activity Log</Text>
									<Text style={styles.settingSub}>View your volunteering history</Text>
								</View>
