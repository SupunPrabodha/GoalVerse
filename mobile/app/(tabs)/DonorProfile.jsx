	...
	useEffect(() => {
		...
		async function load() {
			try {
				...
				// Fetch donor profile
				const res = await fetch(`${API_BASE_URL}/donor/me`, {
				...
				{loading ? (
					...
					<View style={styles.card}>
						...
						<View style={styles.row}>
							<Image
								source={donorAvatar ? { uri: donorAvatar } : require("../../assets/images/logo.png")}
								style={styles.avatar}
							/>
						...
						<View style={styles.section}>
							<Text style={styles.sectionTitle}>{translations[language].totalDonations}</Text>
							<Text style={styles.sectionText}>{donationCount !== null ? donationCount : "—"}</Text>
						</View>
						<View style={styles.section}>
							<Text style={styles.sectionTitle}>{translations[language].totalContributed}</Text>
							<Text style={styles.sectionText}>{totalContributed !== null ? `$${(totalContributed/100).toFixed(2)}` : "—"}</Text>
						</View>
						{/* ...existing code... */}
						{Array.isArray(profile?.interests) && profile.interests.length > 0 && (
							<View style={styles.section}>
								<Text style={styles.sectionTitle}>{translations[language].interests}</Text>
						...
						<View style={[styles.settings]}> 
							...
							<TouchableOpacity style={styles.settingRow} onPress={() => router.push("/(tabs)/DonorHome") }>
								<View>
									<Text style={styles.settingTitle}>{translations[language].donationHistory}</Text>
									<Text style={styles.settingSub}>{translations[language].viewDonations}</Text>
								</View>
